...
<div id="locale">
	...
	<div class="content">
		<h1>Jy die kamer bespreek:</h1>
		<br /> <br />
		<h2>Eerste naam:</h2>
		<%= session.getAttribute("name") %>
		<h3>Kamers beskikbaar :</h3>
	...
	if(request.getAttribute("rooms") != null){
		%>
		<h2>Jy kan kies van die volgende kamers</h2>
...
if(x<hoursList.size()){
	...
	if(list.get(i).getId() == hoursList.get(x).getRoomID()){
			...
			<%-- 	<td id="startHour"><%= hour %>:00</td>
				...
				<td><%= hour %>:00</td> --%>
				<td>
				  <input type="radio" name="getRoom" value="<%= list.get(i).getId()%>/<%= date %> <%= hour-1%>"/>
			...
			<%-- 	<td id="startHour"><%= hour %>:00</td>
				<%hour++; %>
				<td><%= hour %>:00</td> --%>
				<td><input type="radio" name="getRoom" value="<%= list.get(i).getId()%>/<%= date %> <%= hour-1%>"/></td>
			</tr>
			<%
